...
date: "2020-06-17"
title: COVID-19
subtitle: Evolució del coronavirus a Catalunya
summary: Evolució del coronavirus a Catalunya
---
### Catalunya
...
p0 <- ggplot(cat %>% filter(date != max(cat$date))) + geom_line(aes(date, casos), color = "red") + theme_minimal() + geom_vline(aes(xintercept=as.numeric(as.Date("2020-05-04"))), linetype='dashed', color='lightgrey') + geom_vline(aes(xintercept=as.numeric(as.Date("2020-06-19"))), linetype='dashed', color='lightgrey') + labs(x = "", y = "Mitjana de casos a 7 dies")
...
p1 <- ggplot(by_region %>% filter(data != max(by_region$data))) + geom_line(aes(data, casos, color = regio)) + theme_minimal() + geom_vline(aes(xintercept=as.numeric(as.Date("2020-05-04"))), linetype='dashed', color='lightgrey') + geom_vline(aes(xintercept=as.numeric(as.Date("2020-06-19"))), linetype='dashed', color='lightgrey') + labs(x = "", y = "Mitjana de casos a 7 dies", color = "")
...
p2 <- ggplot(by_district %>% filter(data != max(by_district$data))) + geom_line(aes(data, casos, color = districte)) + theme_minimal() + geom_vline(aes(xintercept=as.numeric(as.Date("2020-05-04"))), linetype='dashed', color='lightgrey') + geom_vline(aes(xintercept=as.numeric(as.Date("2020-06-19"))), linetype='dashed', color='lightgrey') + labs(x = "", y = "Mitjana de casos a 7 dies", color = "")
p2 <- ggplotly(p2, width=875, height=600)
p2 <- config(p2, displayModeBar = FALSE)
p2
```
<br>
Font: [Dades Obertes Catalunya](https://analisi.transparenciacatalunya.cat/Salut/Registre-de-casos-de-COVID-19-realitzats-a-Catalun/xuwf-dxjd)