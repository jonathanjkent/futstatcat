...
---
### Catalunya, PCR Positius i Defuncions
...
p0 <- ggplot(cat %>% filter(data != max(cat$data))) + geom_line(aes(data, mitjana, color = tipo)) + theme_minimal() +  labs(x = "", y = "Mitjana a 7 dies", color ="")
p0 <- ggplotly(p0, width=875, height=600)
p0 <- config(p0, displayModeBar = FALSE)
p0
```

### Regió Sanitària, PCR Positius
```{r echo=F,warning=FALSE,message=FALSE}
...
p1 <- ggplot(by_region %>% filter(data != max(by_region$data))) + geom_line(aes(data, casos, color = regio)) + theme_minimal() + labs(x = "", y = "Mitjana de positius PCR a 7 dies", color = "")
...
### Districte de Barcelona, PCR Positius
```{r echo=F,warning=FALSE,message=FALSE}
...
p2 <- ggplot(by_district %>% filter(data != max(by_district$data))) + geom_line(aes(data, casos, color = districte)) + theme_minimal() + labs(x = "", y = "Mitjana de positius PCR a 7 dies", color = "")
