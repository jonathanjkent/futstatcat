...
kable(table, digits = 1, align = 'lccccccccccc', row.names = FALSE, col.names = c('Club', 'Forma', 'Class.', 'Punts', 'Campió', 'UCL', "Europa", "Conf", 'Descens', 'F', 'C', 'Qual.')) %>% add_header_above(c('Temporada' = 4, 'Probabilitat (%)' = 5, 'xGols' = 3)) %>% column_spec(1, bold = T) %>% column_spec(4, border_right = T) %>% column_spec(9, border_right = T)
```
