...
table <- readRDS("~/Google Drive/Futbol/futstat_tables.rds") %>% filter(league == 'primera_1') %>% left_join(teamnames, by = "team") %>% arrange(rank) %>% mutate(league = str_sub(league,1,-3))
