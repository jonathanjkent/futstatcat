...
table <- read.csv("~/Google Drive/Futbol/futstat_tables.csv") %>% filter(league == 'division_honor_juvenil_3') %>% left_join(teamnames, by = "team") %>% arrange(rank) %>% mutate(league = str_sub(league,1,-3))
