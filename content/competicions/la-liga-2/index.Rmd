...
table <- read.csv("~/Google Drive/Futbol/futstat_tables.csv") %>% filter(league == 'segunda2022_1') %>% left_join(teamnames, by = "team") %>% arrange(rank) %>% mutate(league = str_sub(league,1,-3)) %>% left_join(read.csv("~/Google Drive/Futbol/futstat_playoff.csv"), by = "team") %>% mutate(t1 = rowSums(cbind(t1, playoff.ascens), na.rm = T)) 
