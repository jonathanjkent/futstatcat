...
table <- read.csv("~/Google Drive/Futbol/futstat_tables.csv") %>% filter(league == 'primera_division_b_femenina_1') %>% left_join(teamnames, by = "team") %>% arrange(rank) %>% mutate(league = str_sub(league,1,-3))
...
suppressMessages(library(stringr))
teamnames <- read.csv("~/Google Drive/Futbol/teamlist.csv")
table <- read.csv("~/Google Drive/Futbol/futstat_tables.csv") %>% filter(league == 'primera_division_b_femenina_2') %>% left_join(teamnames, by = "team") %>% arrange(rank) %>% mutate(league = str_sub(league,1,-3))
table$linkteam <- iconv(table$fullname,to="ASCII//TRANSLIT")
table$linkteam <- tolower(gsub("[[:punct:]]", "", table$linkteam))
table$linkteam <- gsub("[[:space:]]", "_", table$linkteam)
table$team <- paste0("[", table$shortname, "](/clubs/", table$linkteam,"_",table$league,")")
