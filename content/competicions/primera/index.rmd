...
table <- table %>% select(-shortname,-fullname,-league,-linkteam,-t3,-t4,-des)
kable(table, digits = 1, align = 'lcccccccc', row.names = FALSE, col.names = c('Club', 'Class.', 'Punts', 'Campió', 'UCL', 'Favor', 'Contra', 'Qualitat')) %>% add_header_above(c('Projecció' = 1, 'Mitjana' = 2, 'Probabilitat (%)' = 2, 'xGols' = 3)) %>% column_spec(1, bold = T) %>% column_spec(3, border_right = T) %>% column_spec(5, border_right = T)
```
