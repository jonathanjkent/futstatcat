...
table <- read.csv("~/Google Drive/Futbol/futstat_tables.csv") %>% filter(league == 'primera_division_femenina_1') %>% left_join(teamnames, by = "team") %>% arrange(rank) %>% mutate(league = str_sub(league,1,-3))
