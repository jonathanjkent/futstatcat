...
d <- read.csv('~/Google Drive/Futbol/data_trajectories.csv') %>% filter(league != 'tercera' & team == '/Ue-Castelldefels')
h <- read.csv('~/Google Drive/Futbol/data_tables.csv') %>% filter(league == 'tercera_fase_segunda_rfef')
type1 <- as.numeric(h[2,2] + h[3,2])/2
type2 <- as.numeric(h[7,2] + h[6,2])/2
p <- ggplot(data=d) + geom_line(aes(round,points), size = 1.5, color = 'blue') + geom_hline(aes(yintercept=type1), linetype='dashed', color='#5c5c5c') + annotate('text',0,type1,label = 'Playoff', vjust = -.5, hjust = 'left', color='#5c5c5c') + geom_hline(aes(yintercept=type2), linetype='dashed', color='#5c5c5c') + annotate('text',0,type2,label = 'Fase Descens', vjust = -.5, hjust = 'left', color='#5c5c5c')
