...
traj <- read.csv('~/Google Drive/Futbol/futstat_trajs.csv') %>% filter(team == '/Club-Peluqueria-Mixta-Friol-Femenino') %>% filter(date > as.Date('2021-08-01'))
if (nrow(traj) > 2){
  table <- read.csv('~/Google Drive/Futbol/futstat_tables.csv') %>% filter(league == 'primera_division_b_femenina2020_1')
  league <- read.csv('~/Google Drive/Futbol/ligas.csv') %>% filter(league == 'primera_division_b_femenina2020' & group == 1)
...
hist <- read.csv("~/Google Drive/Futbol/futstat_hists.csv") %>% filter(team == '/Club-Peluqueria-Mixta-Friol-Femenino')
...
games <- read.csv("~/Google Drive/Futbol/futstat_odds.csv")  %>% filter(team1 == '/Club-Peluqueria-Mixta-Friol-Femenino' | team2 == '/Club-Peluqueria-Mixta-Friol-Femenino') %>% left_join(teamnames, by = 'team1') %>% left_join(teamnames %>% rename(team2 = team1, shortname2 = shortname1), by = 'team2') %>% select(shortname1,p1,pd,p2,shortname2) %>% mutate(p1 = p1*100, pd = pd*100, p2 = p2*100)
