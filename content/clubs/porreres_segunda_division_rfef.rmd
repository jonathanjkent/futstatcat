...
traj <- readRDS('~/GoogleDrive/Futbol/futstat_trajs.rds') %>% filter(team == 'porreres-senior')
...
hist <- readRDS('~/GoogleDrive/Futbol/futstat_hists.rds') %>% filter(team == 'porreres-senior')
...
games <- readRDS('~/GoogleDrive/Futbol/futstat_odds.rds')  %>% filter(team1 == 'porreres-senior' | team2 == 'porreres-senior') %>% left_join(teamnames, by = 'team1') %>% left_join(teamnames %>% rename(team2 = team1, shortname2 = shortname1), by = 'team2') %>% select(shortname1,p1,pd,p2,shortname2) %>% mutate(p1 = p1*100, pd = pd*100, p2 = p2*100)
