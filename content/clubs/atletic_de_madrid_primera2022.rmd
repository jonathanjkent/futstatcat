...
if (nrow(traj) > 2){
  table <- read.csv('~/Google Drive/Futbol/futstat_tables.csv') %>% filter(league == 'primera2022_1')
  league <- read.csv('~/Google Drive/Futbol/ligas.csv') %>% filter(league == 'primera2022' & group == 1)
...
p <- ggplot(hist, aes(rank)) + geom_histogram(binwidth = 1, fill = '#004d98')
