...
h <- read.csv('~/Google Drive/Futbol/data_tables.csv') %>% filter(league == 'segundab_2')
type1 <- as.numeric(h[5,2] + h[6,2])/2
type2 <- as.numeric(h[4,2] + h[3,2])/2
p <- ggplot(data=d) + geom_line(aes(round,points), size = 1.5, color = 'blue') + geom_hline(aes(yintercept=type1), linetype='dashed', color='#5c5c5c') + annotate('text',0,type1,label = 'Permanència', vjust = -.5, hjust = 'left', color='#5c5c5c') + geom_hline(aes(yintercept=type2), linetype='dashed', color='#5c5c5c') + annotate('text',0,type2,label = 'Top 3', vjust = -.5, hjust = 'left', color='#5c5c5c')
